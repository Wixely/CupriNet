...
| `WebDebug` | `CUPRINET_LODESTAR_WebDebug` | `false` | Add a debug panel showing exactly what fields go into the minted Intonation (link). Off by default — when off, that data is never sent to the page (not just hidden). Diagnostics only. |
...
Set `WebDebug=true` to add a **debug panel** that breaks down exactly what fields go into the minted Intonation
(version, concordium, Sigil/Seal key, fingerprint, beacons, litany, timestamps, nonce, petition presence, moniker).
It is **off by default**, and when off the server does not send that breakdown to the page at all — it is never put
on the wire, not merely hidden. Diagnostics only.
